@switch(request('sort'))
    @case($nested ? [$nested => $field] : $field)
        <a href="{{ route($route['name'], array_merge($route['params'], ['sort' => $nested ? [$nested => '-'.$field] : '-'.$field])) }}" class="text-decoration-none">
            {{ $title }}
    ...
    @case($nested ? [$nested => '-'.$field] : '-'.$field)
        <a href="{{ route($route['name'], $route['params']) }}" class="text-decoration-none">
            {{ $title }}
