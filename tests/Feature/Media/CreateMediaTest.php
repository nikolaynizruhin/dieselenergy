...
class CreateMediaTest extends TestCase
{
    ...
    public function user_can_visit_create_media_page()
    {
        ...
        $this->actingAs($user)
            ->get(route('medias.create', ['product_id' => $product->id]))
            ->assertViewIs('medias.create');
    }
    ...
    public function guest_cant_create_media()
    {
        ...
        $this->post(route('medias.store'), $media)
            ->assertRedirect(route('login'));
    }
    ...
    public function user_can_create_media()
    {
        ...
        $this->actingAs($user)
            ->post(route('medias.store'), $media)
            ->assertRedirect(route('products.show', $media['product_id']))
    ...
    public function user_cant_create_existing_media()
    {
        ...
        $this->actingAs($user)
            ->post(route('medias.store'), $media->toArray())
            ->assertSessionHasErrors('product_id');
    }
}
